...
namespace GerenciadorHotel.Controllers
{
    ...
    public class AcomodacoesController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Nome,Descricao,QuantidadeCamasCasal,QuantidadeCamasSolteiro,QuantidadeMaximaHospedes,Preco,MinimoNoites,Status,Ativa")] Acomodacao acomodacao)
        {
        ...
        [ValidateAntiForgeryToken]
        [Authorize(Roles = "Administrador")]
        public async Task<IActionResult> Edit(int id, [Bind("Id,Nome,Descricao,QuantidadeCamasCasal,QuantidadeCamasSolteiro,QuantidadeMaximaHospedes,Preco,MinimoNoites,Status,ImagemPrincipalUrl,Ativa,DataCriacao")] Acomodacao acomodacao)
        {
            if (id != acomodacao.Id)
            {
                return NotFound();
